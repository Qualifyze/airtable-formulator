...
function findTableWithHeader($: JQueryElement, header: string): JQueryElement {
    return ($ as unknown as typeof jQuery)(`table tbody:has(tr:first:contains("${header}"))`) as JQueryElement;
}


function getTableColumnContents($table: JQueryElement, column: number): string[] {
    return $table.find("tr:not(:first)").find(`td:eq(${column})`).map(function () {
...
function isSane({ functions, operators } : { functions: string[], operators: string[] }): boolean {
    const expectedFunctions = ["AND", "NOT", "NOW", "ABS"];
    const expectedOperators = ["&", ">", "+"];

    return expectedFunctions.every(fn => functions.includes(fn)) && expectedOperators.every(op => operators.includes(op));
}
...
export function parseReference(html: string): AirtableReference {
    ...
    if (!isSane({functions, operators})) {
        throw new Error(`Parsed Reference failed sanity check: ${JSON.stringify({functions, operators}, null, 2)}`);
    }
