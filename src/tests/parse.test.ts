...
describe("parse", () => {
  ...
  it("Should validate the expression", () => {
    expect(() =>
      parse("thisFnDoesntExist()", { validate: true })
    ).toThrowErrorMatchingInlineSnapshot(`"Parsed formula failed validation"`);
  });

  it("should skip validation", () => {
    expect(() =>
      parse("thisFnDoesntExist()", { validate: false })
    ).not.toThrow();
  });
});
